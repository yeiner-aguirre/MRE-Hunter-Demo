# MRE Hunter Premium Demo

Landing page moderna inspirada en la experiencia visual del sitio oficial de **MRE Hunter**, desarrollada como prueba técnica con una propuesta más sólida, elegante y premium.  
Este proyecto fue creado con mucho amor, dedicación y atención al detalle por **Yeiner Estiven Aguirre Quirama**, **desarrollador web full-stack**.

## Descripción

Esta demo busca recrear y elevar la esencia visual de la marca MRE Hunter, combinando una interfaz moderna, una estructura escalable y una experiencia de usuario más refinada.  
El resultado es una landing page construida con tecnologías actuales del ecosistema frontend, enfocada en rendimiento, mantenibilidad y estética.

## Stack

- React 19
- Vite 8
- Tailwind CSS 4
- React Router 7
- Lucide React

## Scripts

```bash
npm install
npm run dev
npm run build
```

## Arquitectura

```text
src/
  app/            # composición principal y router
  components/     # layout, ui y utilidades visuales
  data/           # navegación, copy y configuración de rutas
  lib/            # helpers compartidos
  pages/          # home, placeholders y 404
  sections/       # bloques de la landing
```

## Notas de la demo

- Se reutilizan assets públicos del sitio oficial para conservar identidad de marca.
- El popup inicial del GPS Pet se replica con cierre y persistencia por sesión.
- Los enlaces internos secundarios abren páginas placeholder listas para crecer después.
- La propuesta mantiene la paleta negro, rojo y blanco, pero con una dirección visual más editorial, elegante y premium.
